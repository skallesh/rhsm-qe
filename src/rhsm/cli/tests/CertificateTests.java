...
//			if (contentNamespace.gpgKeyUrl!=null)				{Assert.assertNotNull(contentNamespace.gpgKeyUrl,				"Mandatory ContentNamespace OID 1.3.6.1.4.1.2312.9.2."+contentNamespace.hash+".1.7 (GPG Key URL) is present with value '"+ contentNamespace.gpgKeyUrl+"'");}						else {if ("content-emptygpg".equals(contentNamespace.name)||"content-nogpg".equals(contentNamespace.name)) {log.info("Skipping assertion of Mandatory ContentNamespace OID 1.3.6.1.4.1.2312.9.2."+contentNamespace.hash+".1.7 (GPG Key URL) for negative test content '"+contentNamespace.name+"'");} else {log.warning("Mandatory ContentNamespace OID 1.3.6.1.4.1.2312.9.2."+contentNamespace.hash+".1.7 (GPG Key URL) is missing"); allMandatoryOIDsFound = false;}}
	...
	protected List<List<Object>> getVerifyBaseRHELProductCertVersionFromEachCDNReleaseVersion_TestDataAsListOfLists() {
		...
		for (String release : clienttasks.getCurrentlyAvailableReleases(null, null, null)) {
			...
			if (release.matches("7.2|7Server") && clienttasks.variant.equals("Server") && clienttasks.arch.equals("s390x")) bugIds.add("1356738"); 	// Bug 1356738 - cdn.redhat.com has the wrong repodata/productId version at server/7/7.2/s390x and server/7/7Server/s390x
			if (release.matches("7.2|7Server") && clienttasks.variant.equals("Server") && clienttasks.arch.equals("ppc64")) bugIds.add("1356740"); 	// Bug 1356740 - cdn.redhat.com has the wrong repodata/productId version at server/7/7.2/ppc64 and server/7/7Server/ppc64
			if (release.matches("7.2|7Workstation") && clienttasks.variant.equals("Workstation") && clienttasks.arch.equals("x86_64")) bugIds.add("1356710"); 	// Bug 1356710 - cdn.redhat.com has the wrong repodata/productId version at workstation/7/7.2 and workstation/7/7Workstation
			if (release.matches("7.2|7ComputeNode") && clienttasks.variant.equals("ComputeNode") && clienttasks.arch.equals("x86_64")) bugIds.add("1356722"); 	// Bug 1356722 - cdn.redhat.com has the wrong repodata/productId version at computenode/7/7.2 and computenode/7/7ComputeNode
			if (release.matches("7.2|7Client") && clienttasks.variant.equals("Client") && clienttasks.arch.equals("x86_64")) bugIds.add("1356729"); 	// Bug 1356729 - cdn.redhat.com has the wrong repodata/productId version at client/7/7.2 and client/7/7Client
