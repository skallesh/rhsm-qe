...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		lang = "ta_IN"; output = "விருப்பங்கள்:";	helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"1074568","1117374"}), lang, command+" "+module+" "+helpOption, 0, newList(output.replaceAll("\\[", "\\\\[").replaceAll("\\]", "\\\\]")+"$")}));
		lang = "te_IN"; output = "ఐచ్చికాలు:";/*The options are:"ఐచ్చికములు:";*/	helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"1074568","1117374"}), lang, command+" "+module+" "+helpOption, 0, newList(output.replaceAll("\\[", "\\\\[").replaceAll("\\]", "\\\\]")+"$")}));
