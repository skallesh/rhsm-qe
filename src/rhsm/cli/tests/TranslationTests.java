...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		lang = "de_DE"; usage = "(Verwendung|Verbrauch): subscription-manager register [OPTIONEN]";		helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"693527","839807","845304"}), lang, command+" "+helpOption+" "+module, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
		...
		lang = "de_DE"; usage = "(Verwendung|Verbrauch): rhn-migrate-classic-to-rhsm [OPTIONEN]";		helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"707080"}), lang, command+" "+helpOption, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
		...
		lang = "pt_BR"; usage = "Uso:\\n  rhsmcertd [OPÇÃO...]";				helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {null, lang, command+" "+helpOption, 0, newList(usage.replaceAll("\\.","\\\\.").replaceAll("\\[(.+)\\]", "\\\\[($1|OPTION\\\\.\\\\.\\\\.)\\\\]")+"\\s*$")}));
