...
public class TranslationTests extends SubscriptionManagerCLITestScript {
	...
	@Test(	description="verify that only the expected rhsm.mo tranlation files are installed for each of the supported locales",
			groups={"AcceptanceTests", "blockedByBug-871152", "blockedByBug-912460", "blockedByBug-1003017"},
...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		lang = "ta_IN"; usage = "பயன்பாடு: subscription-manager register [விருப்பங்கள்]";					helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"839807","845304"}), lang, command+" "+helpOption+" "+module, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
		...
		lang = "ta_IN"; usage = "பயன்பாடு: rhn-migrate-classic-to-rhsm [விருப்பங்கள்]";						helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {null, lang, command+" "+helpOption, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
	...
	protected List<List<Object>> getInvalidRegistrationWithLocalizedStringsAsListOfLists(){
		...
		if (sm_serverType.equals(CandlepinType.standalone)) {
			...
			ll.add(Arrays.asList(new Object[]{null,								"pa_IN.UTF-8", sm_clientUsername+getRandInt(), sm_clientPassword+getRandInt(), 255, null, "ਗਲਤ ਕਰੀਡੈਂਸ਼ੀਅਲਸ"}));	// "ਗਲਤ ਕਰੀਡੈਂਸ਼ਲ"
