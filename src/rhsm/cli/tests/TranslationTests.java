...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		lang = "te_IN"; output = "";													helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"1074568"}), lang, command+" "+helpOption, 0, newList(output.replaceAll("\\[", "\\\\[").replaceAll("\\]", "\\\\]").replaceAll(" ", "\\\\s+")+"$")}));
