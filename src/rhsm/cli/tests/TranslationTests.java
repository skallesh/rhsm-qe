...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		lang = "kn_IN"; usage = "ಬಳಕೆ: subscription-manager MODULE-NAME [ಮಾಡ್ಯೂಲ್-ಆಯ್ಕೆಗಳು] [--help]";						helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug("811294"), lang, command+" "+helpOption, helpOption.equals("--help")?0:1, newList(usage.replaceAll("\\[", "\\\\[").replaceAll("\\]", "\\\\]")+"$")}));
		...
		lang = "hi_IN"; usage = "प्रयोग: subscription-manager register [विकल्प]";								helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"839807","845304"}), lang, command+" "+helpOption+" "+module, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
		...
		lang = "hi_IN"; usage = "प्रयोग: rhn-migrate-classic-to-rhsm [विकल्प]";								helpOption=helpOptions.get(randomGenerator.nextInt(helpOptions.size())); ll.add(Arrays.asList(new Object[] {null, lang, command+" "+helpOption, 0, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
