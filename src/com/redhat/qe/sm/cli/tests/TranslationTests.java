...
// debugTesting pofilterTests = Arrays.asList("newlines");
	...
	protected List<List<Object>> getTranslatedCommandLineHelpDataAsListOfLists() {
		...
		for (String smHelpCommand : new String[]{clienttasks.command+" -h",clienttasks.command+" --help"}) {
			...
			lang = "zh_CN"; usage = "用法：subscription-manager register [选项]";						ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"839807","845304"}), lang, smHelpCommand+" "+module, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
			...
			lang = "mr_IN"; usage = "वपार: subscription-manager register [पर्याय]";						ll.add(Arrays.asList(new Object[] {new BlockedByBzBug(new String[]{"839807","845304"}), lang, smHelpCommand+" "+module, newList(usage.replaceAll("\\[(.+)\\]", "\\\\[($1|OPTIONS)\\\\]")+"$")}));
