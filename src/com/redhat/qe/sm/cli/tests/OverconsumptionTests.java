...
//debugging if (pool.productId.equals("awesomeos-virt-4") && pool.quantity.equals("5")) {testPool=pool;break;}
		...
		for (int i=quantity; i>0; i--) {
			...
			systemConsumerIds.add(client1tasks.getCurrentConsumerId(client1tasks.register(sm_clientUsername, sm_clientPassword, sm_clientOrg, null, null, registereeName, null, null, (String)null, null, false, null, null, null)));
...
//		Assert.assertNotNull(pool, "Found the test pool amongst --all --available after having consumed all of its available entitlements.");
	...
	public void ConcurrentAttemptToSubscribe_Test() throws JSONException, Exception {
		...
		client1tasks.clean(null,null,null);
		client1tasks.register(sm_clientUsername,sm_clientPassword,null,null,null,registereeName,systemConsumerIds.get(0), null, (String)null, null, false, null, null, null);
		...
		client2tasks.register(sm_clientUsername,sm_clientPassword,null,null,null,registereeName,systemConsumerIds.get(1), null, (String)null, null, false, null, null, null);
...
//		Assert.assertEquals(result1.getStdout().trim(), "","The lack of information in stdout from the subscribe command on '"+client1tasks.hostname+"' indicates that it successfully subscribed to poolid: "+testPool.poolId);
		Assert.assertEquals(result1.getStdout().trim(), clienttasks.msg_SuccessfulSubscribe+testPool.poolId,"On '"+client1tasks.hostname+"' we successfully subscribed to poolid: "+testPool.poolId);
...
//		Assert.assertEquals(result2.getStdout().trim(), "","The lack of information in stdout from the subscribe command on '"+client2tasks.hostname+"' indicates that it successfully subscribed to poolid: "+testPool.poolId);
		Assert.assertEquals(result2.getStdout().trim(), clienttasks.msg_SuccessfulSubscribe+testPool.poolId,"On '"+client2tasks.hostname+"' we successfully subscribed to poolid: "+testPool.poolId);
...
//		Assert.assertEquals(pool.quantity, "0", "Asserting the test pool quantity has dropped by 2 due to the concurrent call to subscribe.");
	...
	public void ConcurrentAttemptToOversubscribe_Test() throws JSONException, Exception {
		...
		client2tasks.clean(null,null,null);
		systemConsumerIds.add(client2tasks.getCurrentConsumerId(client2tasks.register(sm_clientUsername, sm_clientPassword, sm_clientOrg, null, null, registereeName, null, null, (String)null, null, false, null, null, null)));
...
//		Assert.assertEquals(sshWinner.getStdout().trim(), "","The lack of information in stdout from the subscribe command on '"+smtWinner.hostname+"' indicates that it won the subscribe race to the subscription pool's final entitlement.");
		Assert.assertEquals(sshWinner.getStdout().trim(), clienttasks.msg_SuccessfulSubscribe+testPool.poolId,"On '"+smtWinner.hostname+"' we successfully subscribed to poolid '"+testPool.poolId+"' indicating that it won the subscribe race to the subscription pool's final entitlement.");
	...
	public void unsubscribeAndUnregisterMultipleSystemsAfterClass() {
		...
		if (client1tasks!=null) {
			...
			for (String systemConsumerId : systemConsumerIds) {
				client1tasks.register_(sm_clientUsername,sm_clientPassword,null,null,null,null,systemConsumerId, null, (String)null, Boolean.TRUE, null, null, null, null);
