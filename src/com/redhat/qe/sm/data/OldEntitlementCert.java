...
public class OldEntitlementCert extends AbstractCommandLineData {
	...
	public OldEntitlementCert(String key, Map<String, String> certData){
		super(certData);
		this.key = key;
	}
...
//		public String enabled;
	 ...
	 * @return
	 */
	static public List<OldEntitlementCert> parse(String certificates) {
...
//		regexes.put("enabled",			"1\\.3\\.6\\.1\\.4\\.1\\.2312\\.9\\.2\\.(\\d+)\\.1\\.8:[\\s\\.\\cM]*([a-zA-Z_0-9-]+)");
		...
		List<OldEntitlementCert> entitlementCerts = new ArrayList<OldEntitlementCert>();
		for(String certKey : productMap.keySet())
			entitlementCerts.add(new OldEntitlementCert(certKey, productMap.get(certKey)));
